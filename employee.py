...
class Commission(Employee):
    ...
    def __str__(self):
        if self.monthly:
            ...
            if self.contract:
                return f"{self.name} works on a monthly salary of {self.monthly} and receives a commission for {self.contract} contract(s) at {self.contract_pay}/contract. Their total pay is {self.get_pay()}."
        else:
            if self.bonus:
                return f"{self.name} works on a contract of {self.hours_worked} hours at {self.hourly}/hour and receives a bonus commission of {self.bonus}. Their total pay is {self.get_pay()}."
            if self.contract:
                return f"{self.name} works on a contract of {self.hours_worked} hours at {self.hourly}/hour and receives a commission for {self.contract} contract(s) at {self.contract_pay}/contract. Their total pay is {self.get_pay()}."
...
renee = Commission('Renee', monthly=3000, contract=4, contract_pay=200)

# Jan works on a contract of 150 hours at 25/hour and receives a commission for 3 contract(s) at 220/contract.  Their total pay is 4410.
jan = Commission('Jan', hourly=25, hours_worked=150, contract=3, contract_pay=220)

# Robbie works on a monthly salary of 2000 and receives a bonus commission of 1500.  Their total pay is 3500.
robbie = Commission('Robbie', monthly=2000, bonus=1500)

# Ariel works on a contract of 120 hours at 30/hour and receives a bonus commission of 600.  Their total pay is 4200.
ariel = Commission('Ariel', hourly=30, hours_worked=120, bonus=600)
